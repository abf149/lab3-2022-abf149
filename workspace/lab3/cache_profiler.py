        ...
        'classes': [{
            ...
            'subcomponents': [{
                ...
                'attributes': {
                    ...
                    'tag_size': 64, 'n_rd_ports': 1,
                    'n_wr_ports': 1, 'n_rdwr_ports': 1,
                    'n_banks': 1, 'data_latency': 2,
                    'mshr_size': 4,
                    'write_buffer_size': 8,
                    'clockrate': 999}}, {
                'name': 'DRAM',
            ...
            'actions': [{
                ...
                'subcomponents': [{
                    'name': 'cache',
                    'actions': [{'name': 'read_access'}]}]}, {
                'name': 'write_hit',
                'subcomponents': [{
                    'name': 'DRAM', 'actions': [{'name': 'write'}]}, {
                    'name': 'cache',
                    'actions': [{'name': 'write_access'}]}]}, {
                'name': 'read_miss',
                'subcomponents': [{
                    'name': 'cache',
                    'actions': [{'name': 'read_access'}]}, {
...
class CacheProfiler(object):
    def __init__(self,
        ...
        self.cache['compound_components']['classes'][0][
            'subcomponents'][0]['attributes']['n_rdwr_ports'] = n_rdwr_ports
        self.cache['compound_components']['classes'][0][
    ...
    def profile(self, action_count):
        ...
        cwd = self.run_dir
        cmd = 'accelergy arch.yaml cache.yaml action_counts.yaml > /dev/null 2>&1'
